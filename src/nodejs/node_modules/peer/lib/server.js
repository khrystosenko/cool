...
var LIMIT_PER_ROOM = 3;

var app = exports = module.exports = rooms = {};
...
app._initializeWSS = function(server) {
  ...
  this._wss.on('connection', function(socket) {
    ...
    var room_id = self._get_room_id(id);

    if (rooms[room_id]) {
      if (rooms[room_id].length < LIMIT_PER_ROOM) {
        rooms[room_id].push(id);
      } else {
        socket.send(JSON.stringify({ type: 'ERROR', payload: { msg: {code: 401}} }));
        socket.close();
        return;
      }
    } else {
      ...
      self._checkKey(key, ip, function(err) {
        if (!err) {
          if (!self._clients[key][id]) {
            ...
            self._ips[ip]++;
            socket.send(JSON.stringify({ type: 'OPEN', payload: {room: rooms[room_id]}}));
          }
...
app._startStreaming = function(res, key, id, token, open) {
  ...
  if (open) {
    res.write(JSON.stringify({ type: 'OPEN', payload: {room: rooms[app._get_room_id(id)]}}) + '\n');
  }
...
app._removePeer = function(key, id) {
  if (this._clients[key] && this._clients[key][id]) {
    var room_id = this._get_room_id(id);
    var index = rooms[room_id].indexOf(id);
    if (index > -1) {
